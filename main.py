...
shap.summary_plot(shap_values_xgb, X_train_scaled)
plt.savefig('plots/explainer/xgb_summary_plot.png')
plt.close()

# Force Plot for a single prediction
print("SHAP force plot for a single prediction - XGBoost")
shap.force_plot(shap_values_xgb[0], matplotlib=True)
plt.savefig('plots/explainer/xgb_force_plot.png')
plt.close()

# Waterfall Plot for a single prediction
print("SHAP waterfall plot for a single prediction - XGBoost")
shap.waterfall_plot(shap_values_xgb[0])
plt.savefig('plots/explainer/xgb_waterfall_plot.png')
plt.close()
